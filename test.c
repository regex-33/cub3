...
void	move_player(t_mlx *mlx, double move_x, double move_y)	// move the player
{
	...
	new_x = roundf(mlx->ply->plyr_x + move_x); // get the new x position | roundf is function that return the nearest integer value
...
int	get_color(t_mlx *mlx, int flag)	// get the color of the wall
{
	...
	if (flag == 0)
	{
		if (mlx->ray->ray_ngl > M_PI / 2 && mlx->ray->ray_ngl < 3 * (M_PI / 2)) // check the angle if is not between 90 and 270
			return (0xB5B5B5FF); // west wall
		else
			return (0xB5B5B5FF); // east wall // if the angle is between 90 and 270
	}
	else
	{
		if (mlx->ray->ray_ngl > 0 && mlx->ray->ray_ngl < M_PI) // check the angle if is not between 0 and 180
			return (0xF5F5F5FF); // south wall
		else
			return (0xF5F5F5FF); // north wall // if the angle is between 0 and 180
	}
}
...
void	render_wall(t_mlx *mlx, int ray)	// render the wall
{
	...
	wall_h = (TILE_SIZE / mlx->ray->distance) * ((S_W / 2) / tan(mlx->ply->fov_rd / 2)); // get the wall height // S_W / 2 = 950 we use tan(30 / 2) = 0.57735026919
...
int	inter_check(float angle, float *inter, float *step, int is_horizon)	// check the intersection
{
	if (is_horizon)
	{
		if (angle > 0 && angle < M_PI) // check the angle if is not between 0 and 180
		{
			*inter += TILE_SIZE;
			return (-1);
		}
	...
	{
		if (!(angle > M_PI / 2 && angle < 3 * M_PI / 2)) // check the angle if is not between 90 and 270
		{
			*inter += TILE_SIZE;
			return (-1);
		}
...
float	get_h_inter(t_mlx *mlx, float angl)	// get the horizontal intersection
{
	...
	y_step = TILE_SIZE;
	x_step = TILE_SIZE / tan(angl); // why tan(angl) because we need to get the x_step value it's the distance between the x position in the map and the next x position in the map
	h_y = floor(mlx->ply->plyr_y / TILE_SIZE) * TILE_SIZE;  // get the y position in the map in pixels in the center of the tile | floor is function that return the largest integer value less than or equal to x
	pixel = inter_check(angl, &h_y, &y_step, 1); // check the intersection and get the pixel value
	h_x = mlx->ply->plyr_x + (h_y - mlx->ply->plyr_y) / tan(angl); // get the x position in the map in pixels in the center of the tile using the angle and the y position in the map
...
void	cast_rays(t_mlx *mlx)	// cast the rays
{
	...
	ray = 0;
	mlx->ray->ray_ngl = mlx->ply->angle - (mlx->ply->fov_rd / 2); // the start angle // first one is 30
...
void init_the_player(t_mlx mlx)	// init the player structure
{
	...
	mlx.ply->fov_rd = (FOV * M_PI) / 180; // field of view in radians // 60 * M_PI / 180 = 1.04719755 // mpi = 3.14159265 // 1.04719755 * 3 = 3.14159265
